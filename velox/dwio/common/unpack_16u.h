...
/**
 * @brief Contains implementation of functions for unpacking 9..16-bit data to
 * words
 * @date 08/02/2020
 *
...
inline const uint8_t* unpack_Nu16u(
...
inline const uint8_t* unpack_Nu16u(
  ...
  if (bit_width > bits_in_buf) {
    next_word =
        (uint32_t)(bit_width - bits_in_buf > 8u ? *src16u_ptr : *((uint8_t*)src16u_ptr));
    next_word = next_word << bits_in_buf;
    src = src | next_word;
  }
  *dst16u_ptr = (uint16_t)(src & mask);
...
template <typename OutType>
const uint8_t*
unpack_9u16u(const uint8_t* src_ptr, uint32_t values_to_read, OutType* dst_ptr);

template <>
inline const uint8_t* unpack_9u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    uint16_t* dst_ptr) {
...
template <typename OutType>
inline const uint8_t* unpack_10u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
template <typename OutType>
inline const uint8_t* unpack_11u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
template <typename OutType>
inline const uint8_t* unpack_12u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
template <typename OutType>
inline const uint8_t* unpack_13u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
template <typename OutType>
inline const uint8_t* unpack_14u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
template <typename OutType>
inline const uint8_t* unpack_15u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
template <typename OutType>
inline const uint8_t* unpack_16u16u(
    const uint8_t* src_ptr,
    uint32_t values_to_read,
    OutType* dst_ptr);
...
inline const uint8_t* unpack_16u16u(
  ...
  if (values_to_read >= 32u) {
    while (values_to_read >= 32u) {
      __m512i src = _mm512_loadu_epi16(src16u_ptr);
...
inline const uint8_t* unpack_16u16u(
  ...
  if (values_to_read >= 32u) {
    while (values_to_read >= 32u) {
      __m512i src = _mm512_loadu_epi8(src16u_ptr);
