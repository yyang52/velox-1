...
void convertToArrowImpl(
  ...
  const void** buffers =
      (const void**)malloc(sizeof(void*) * arrowArray.n_buffers);
...
void convertArrowSchema(std::string col_type, ArrowSchema& arrowSchema) {
  arrowSchema.format = getArrowFormat(col_type);
  ...
  arrowSchema.release = releaseSchema;
}
...
void convertToArrow(
    ...
    std::string col_type,
    int num_rows) {
  convertArrowSchema(col_type, arrowSchema);
  const char* arrow_type = arrowSchema.format;
  switch (arrow_type[0]) {
    case 'b':
